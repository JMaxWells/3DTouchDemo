...
/**
 准备百川服务
 <使用方式:请在webview init之前调用>
 (SDK会在app UserAgent里追加百川标识)
 */
- (void)prepareService;

/**
 移除百川服务
 <使用方式:请在webview的ViewController的viewDidDisappear里调用>
 (SDK会删除之前在UserAgent追加的百川标识，IOS的WebView共享同一个全局UserAgent)
 */
- (void)releaseService;
 ...
 */
- (BOOL)isAllowLoadURLRequest:(NSURL *)url
